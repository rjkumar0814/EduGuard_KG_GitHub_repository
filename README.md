...
the complete 109,816-row student-day dataset;
the complete deterministic rule used to generate the 12,723 positive
infectiousness labels;
the held-out model probability vectors;
threshold-level ROC/PR coordinates;
bin-level calibration data;
window-level temporal stability values;
condition-level robustness values;
full bootstrap replicate outputs.
